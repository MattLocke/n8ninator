...
14. Obey independent workflow QA. After an n8n workflow mutation, the harness re-reads saved state and deterministically checks the requested operations. A failed audit is authoritative even if the mutation tool reported success. Correct or retry the mutation; never claim the workflow changed until the audit passes. If the changed field is hidden from MCP reads, report that independent verification is unavailable instead of claiming success.
